...
function Steps({ children, step, setStep, setStepCount, setStepIsIntro, chapter }) {
...
function App() {
  ...
  /* Weiter: erst durch die Übungen, dann ins nächste Kapitel */
  const next = () => {
  ...
  /*
    Vorschau zeichnen, sobald die Zeichenfläche wirklich da ist.
    Sie erscheint erst auf der Übung, in der das Bild steht – deshalb
    muss auch der Übungswechsel ein Neuzeichnen auslösen.
  */
  useEffect(() => {
    if (screen !== 14) return;
    let cancelled = false;
    ...
    const zeichne = () => {
      if (cancelled) return;
      if (posterRef.current) { drawPoster(posterRef.current); return; }
      if (versuche++ < 12) timer.push(setTimeout(zeichne, 80));
    };
    timer.push(setTimeout(zeichne, 40));
    return () => { cancelled = true; timer.forEach(clearTimeout); };
  ...
  */
  ...
  const Ch1 = () => {
    ...
    return (
      <Steps step={step} setStep={setStep} setStepCount={setStepCount} setStepIsIntro={setStepIsIntro} chapter={screen}>
        <Lead>
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.paper, paddingBottom: 120 }}>
      ...
      {typeof screen === "number" && step === stepCount - 1 && screen < CH.length && chapterFilled(screen) && (
